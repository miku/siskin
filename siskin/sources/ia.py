...
class DownloadFile(IATask):
    ...
    itempath = luigi.Parameter(default="fatcat_bulk_exports_2022-11-24/release_extid.tsv.gz")

    def run(self):
        tmpdir = tempfile.mkdtemp("siskin-", "", "")
        shellout("rclone copy ia:/{itempath} {tmpdir}", item=self.itempath, tmpdir=tmpdir)
