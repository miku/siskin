...
# We don't stop on yum failure (yum seems to return 1 if everything is up to
# date), so check here explictly.
hash curl 2>/dev/null || {
	echo >&2 "curl is required" && exit 1
}
hash wget 2>/dev/null || {
	echo >&2 "wget is required" && exit 1
}

# install_latest_deb installs latest deb release, given a username/repository from github.com.
install_latest_deb() {
	if [ $# -eq 0 ]; then
		echo "install_latest_deb expects a username/repository as argument" && exit 1
	fi

	# If we failed to install jq, complain here.
	hash jq 2>/dev/null || {
		echo >&2 "jq is required" && exit 1
	}

	URL=$(curl -s https://api.github.com/repos/$1/releases | jq '.[0].assets_url' | xargs curl -s | jq -r '.[].browser_download_url' | grep "deb")
	RC=$?
	if [[ $RC != 0 ]]; then
		echo "cannot find latest package for $1, maybe hit API limits?" && exit $RC
	fi

	# dpkg cannot handle urls?
	mkdir -p $TMPDIR && wget -O $TMPDIR/transit.deb "$URL" && dpkg -i $TMPDIR/transit.deb && rm -f $TMPDIR/transit.deb
}

# install_latest_rpm installs latest rpm release, given a username/repository from github.com.
install_latest_rpm() {
	if [ $# -eq 0 ]; then
		echo "install_latest_rpm expects a username/repository as argument" && exit 1
	fi
...
centos_6_install_python_27() {
	...
	cd $TMPDIR
	wget -O Python-$PYVERSION.tar.xz https://www.python.org/ftp/python/$PYVERSION/Python-$PYVERSION.tar.xz
...
if [ -f /etc/debian_version ]; then
	...
	install_latest_deb "miku/filterline"
	install_latest_deb "miku/groupcover"
...
if [ -f /etc/redhat-release ]; then
	...
	if [[ ! ("$VERSION" == "6" || "$VERSION" == "7") ]]; then
		echo "only CentOS 6 and 7 supported" && exit 1
	fi

	# Centos 6 needs special treatment.
	if [[ "$VERSION" == "6" ]]; then
	...
	install_latest_rpm "miku/groupcover"
...
hash pip 2>/dev/null || {
	echo >&2 "pip is required. On Centos, python-pip is in EPEL." && exit 1
}
...
download_file_from_github() {
	...
	if [ "$#" -eq 0 ]; then
		echo "download_config_file_from_github expects a path as argument, which is appended to $BASE" && exit 1
	fi
...
==========================================================================================
Please review configuration paths, hosts, ports, credentials, oai-endpoints configuration.

	~/.config/siskin/siskin.ini or /etc/siskin/siskin.ini
