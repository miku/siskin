...
class CreateConfiguration(Task):
    ...
    def run(self):
        with open(
            os.path.join(self.inputdir(), "licensing/filterconfig.json.template")
        ) as handle:
...
class ApplyLicensing(Task):
    ...
    def requires(self):
        return {"config": CreateConfiguration(), "input": CombinedIntermediateSchema()}
